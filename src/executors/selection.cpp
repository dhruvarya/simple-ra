...
void executeSELECTION()
{
    ...
    // if(table.indexingStrategy == HASH && table.indexedColumn == firstColumnIndex && parsedQuery.selectionBinaryOperator == EQUAL && parsedQuery.selectType == INT_LITERAL) {
    //     int value = parsedQuery.selectionIntLiteral;
    //     int pg = table.hashtable->getRecord(value).second;
    //     Cursor indexCurs(table.tableName, pg);
    //     cursor = indexCurs;
    //     row = cursor.getNext();
    // } 
    
    // if(table.indexingStrategy == BTREE && table.indexedColumn == firstColumnIndex && parsedQuery.selectionBinaryOperator == EQUAL && parsedQuery.selectType == INT_LITERAL) {
    //     int value = parsedQuery.selectionIntLiteral;
    //     int pg = table.bptree->getRecord(value).second;
    //     Cursor indexCurs(table.tableName, pg);
    //     cursor = indexCurs;
    //     row = cursor.getNext();
    // }
